...
import com.sscl.blelibraryforkotlin.databinding.DialogSetStartsNameFilterBinding
 ...
 * 设置名称过滤-匹配名称开头 的对话框
 */
class SetStartNameFilterDialog(context: Context, private val bleScanner: BleScanner) :
    BaseDataBindingDialog<DialogSetStartsNameFilterBinding>(context) {
    /**
     * 设置布局
     */
    override fun setLayout(): Int {
        return R.layout.dialog_set_starts_name_filter
    }
    ...
    private val onClickListener = View.OnClickListener {
        when (it.id) {
            binding.addNewBtn.id -> {
                showAddFilterStartsNameDialog()
                dismiss()
            }
            binding.clearBtn.id -> {
                bleScanner.clearFilterStartsNames()
    ...
    private fun initFilterRvData() {
        ...
        stringListRecyclerViewAdapter.addData(bleScanner.getFilterStartsNames())
    }

    /**
     * 显示添加设备名过滤-匹配开头的对话框
     */
    private fun showAddFilterStartsNameDialog() {
        val view = View.inflate(context, R.layout.view_add_filter_starts_name, null)
        AlertDialog.Builder(context)
            .setTitle(R.string.add_new_filter)
            .setView(view)
            .setPositiveButton(R.string.confirm) { _, _ ->
                val text =
                    view.findViewById<EditText>(R.id.filter_starts_name_et).text.toString()
                if (text.isEmpty()) {
                    context.toastL(R.string.data_empty)
                    return@setPositiveButton
                }
                bleScanner.addFilterStartsName(text)
    ...
    private fun showConfirmDeleteDialog(value: String) {
        AlertDialog.Builder(context)
            ...
            .setPositiveButton(R.string.confirm) { _, _ ->
                bleScanner.removeFilterStartsName(value)
